"""Load batch audit inputs from JSON or JSONL files."""
...
def _normalize_item(raw: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    if not isinstance(context, str) or not isinstance(answer, str):
        raise BatchInputError(f"Record {index}: 'context' and 'answer' must be strings")
    record_id = raw.get("id")
    if record_id is None:
        record_id = str(index)
    query = raw.get("query")
    if query is not None and not isinstance(query, str):
        raise BatchInputError(f"Record {index}: 'query' must be a string when provided")
    return {
        "id": str(record_id),
        "context": context,
        "answer": answer,
